...
impl<'a, K: Display> Subclient<'a, K> {
	...
	///
	/// **Endpoint**: `/lol/platform/v3/masteries/by-summoner/{summoner_id}`
	pub fn get(&self) -> Result<Vec<dto::ChampionMastery>, StatusCode> {
		let path = format!(
			"/lol/platform/v3/masteries/by-summoner/{summoner_id}",
...
mod tests {
	#[test]
	fn get() {
		::CLIENT.platform().masteries().by_summoner(24338059).get().unwrap();
	}
}
