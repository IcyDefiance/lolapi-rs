...
impl<'a, K: Display + Clone> Subclient<'a, K> {
	pub(super) fn new(
		...
		Self { region: region, key: key, app_limit: app_limit, get_master_league_limit: get_master_league_limit }
	}

	pub fn by_queue(&self, queue: QueueType) -> by_queue::Subclient<K> {
		by_queue::Subclient::new(self.region, self.key.clone(), &self.app_limit, &self.get_master_league_limit, queue)
	}
}
