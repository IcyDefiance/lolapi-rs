...
impl<'a, K: Display> Subclient<'a, K> {
	...
	pub fn get(&mut self, locale: Option<Locale>, version: Option<&str>) -> Result<dto::LanguageStrings, StatusCode> {
...
mod tests {
	#[test]
	fn get() {
		::CLIENT.lock().unwrap().static_data_v3().language_strings().get(Some(::Locale::en_US), None).unwrap();
	}
}
